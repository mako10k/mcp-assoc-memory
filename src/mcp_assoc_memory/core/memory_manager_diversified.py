...
class MemoryManagerDiversified:
    ...
    async def diversified_similarity_search(
        ...
        try:
            ...
            diverse_results = await self._apply_diversity_filter(
                candidates, scope, diversity_threshold, limit
            )
    ...
    async def _apply_diversity_filter(
        ...
        try:
            ...
            sorted_candidates = sorted(
                candidates,
                key=lambda x: x.get("similarity", x.get("score", 0.0)),
                reverse=True
            )
    ...
    async def diverse_tag_search(
        ...
        try:
            ...
            candidates = await self.search_by_tags(
                tags, scope, match_all=False, limit=limit * 3
            )
    ...
    async def diverse_category_search(
        ...
        try:
            # Get more candidates than needed
            candidates = await self.search_by_category(
                category, scope, limit=limit * 3
            )
            ...
            for depth in range(exploration_depth):
                ...
                for memory_id in current_level:
                    ...
                    if memory:
                        ...
                        similar_memories = await self.find_similar_memories(
                            memory_id, limit=limit_per_level * 2
                        )
                        ...
                        for similar_memory, _ in similar_memories:
                            if (len(next_level) < limit_per_level and 
                                ...
                                similar_memory.id not in level_exclude):
                                
                                next_level.append(similar_memory.id)
                                
                                # Add similar memories to level exclude set
                                await self._add_to_exclude_set(
                                    similar_memory, level_exclude, None, diversity_threshold
                                )
