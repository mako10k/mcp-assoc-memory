...
class HistogramMetric:
    ...
    name: str
    description: str
    buckets: List[float] = field(default_factory=lambda: [0.1, 0.25, 0.5, 1.0, 2.5, 5.0, 10.0])
...
class MetricsCollector:
    ...
    def counter(self, name: str, description: str = "", labels: Dict[str, str] = None) -> CounterMetric:
        """カウンターメトリクスを取得または作成"""
    ...
    def gauge(self, name: str, description: str = "", labels: Dict[str, str] = None) -> GaugeMetric:
        """ゲージメトリクスを取得または作成"""
        key = self._generate_key(name, labels or {})
    ...
    def histogram(self, name: str, description: str = "", labels: Dict[str, str] = None, 
    ...
    def time_series_value(self, name: str, value: float, labels: Dict[str, str] = None):
        """時系列値を記録"""
        key = self._generate_key(name, labels or {})
...
class PerformanceMetrics:
    ...
    def record_memory_operation(self, operation: str, duration_ms: float, success: bool = True):
        """記憶操作のメトリクスを記録"""
        # 操作カウント
    ...
    def record_search_operation(self, domain: str, result_count: int, duration_ms: float):
        """検索操作のメトリクスを記録"""
        # 検索カウント
        counter = self.collector.counter(
    ...
    def record_embedding_operation(self, model: str, text_length: int, duration_ms: float):
        """埋め込み生成のメトリクスを記録"""
        # 埋め込み生成カウント
        counter = self.collector.counter(
