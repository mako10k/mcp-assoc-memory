...
class Validator:
    ...
    def validate_memory_content(content: str) -> str:
        ...
        if not isinstance(content, str):
            raise ValidationError(
                "content", content, "Content must be a string")

        if not content.strip():
            raise ValidationError(
                "content", content, "Content cannot be empty")

        if len(content) > 50000:  # 50KB制限
            raise ValidationError(
                "content", content, "Content is too long (max 50KB)")

        return content.strip()

    @staticmethod
    def validate_memory_domain(
            domain: Union[str, MemoryDomain]) -> MemoryDomain:
        """記憶ドメインの検証"""
        if isinstance(domain, str):
            try:
                ...
                raise ValidationError(
                    "domain", domain, f"Invalid domain. Valid domains: {valid_domains}")
        ...
        raise ValidationError(
            "domain",
            domain,
            "Domain must be a string or MemoryDomain enum")
    ...
    def validate_tags(tags: List[str]) -> List[str]:
        ...
        for tag in tags:
            ...
            if len(tag) > 50:
                raise ValidationError(
                    "tags", tag, "Tag is too long (max 50 characters)")
    ...
    def validate_metadata(metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(metadata_json.encode('utf-8')) > 10240:
                raise ValidationError(
                    "metadata", metadata, "Metadata is too large (max 10KB)")
    ...
    def validate_user_id(user_id: Optional[str]) -> Optional[str]:
        ...
        if not isinstance(user_id, str):
            raise ValidationError(
                "user_id", user_id, "User ID must be a string")
        ...
        if not re.match(r'^[a-zA-Z0-9_@.-]+$', user_id):
            raise ValidationError(
                "user_id", user_id, "User ID contains invalid characters")
    ...
    def validate_project_name(name: str) -> str:
        ...
        if not isinstance(name, str):
            raise ValidationError(
                "name", name, "Project name must be a string")
        ...
        if len(name) > 100:
            raise ValidationError(
                "name", name, "Project name is too long (max 100 characters)")
    ...
    def validate_project_role(role: Union[str, ProjectRole]) -> ProjectRole:
        ...
        if isinstance(role, str):
            try:
                ...
                raise ValidationError(
                    "role", role, f"Invalid role. Valid roles: {valid_roles}")

        if isinstance(role, ProjectRole):
            return role

        raise ValidationError(
            "role", role, "Role must be a string or ProjectRole enum")
    ...
    def validate_search_query(query: str) -> str:
        ...
        if not isinstance(query, str):
            raise ValidationError(
                "query", query, "Search query must be a string")
        ...
        if len(query) > 1000:
            raise ValidationError(
                "query", query, "Search query is too long (max 1000 characters)")
    ...
    def validate_limit(limit: Optional[int], max_limit: int = 100) -> int:
        ...
        if limit > max_limit:
            raise ValidationError(
                "limit", limit, f"Limit is too large (max {max_limit})")
