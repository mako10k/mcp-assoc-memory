...
async def google_search_and_store(
    ...
    try:
        ...
        if store_summary:
            ...
            stored_memories.append(
                {"memory_id": summary_memory_id, "type": "search_summary", "result_count": len(data["items"])}
            )
...
async def fetch_url_and_store(
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            async with session.request(
                method, url, headers=request_headers, timeout=aiohttp.ClientTimeout(total=timeout)
            ) as response:
                # Get response info
    ...
    try:
        ...
        main_memory_id = await memory_manager.store_memory(
            content=main_content, scope=scope, metadata=main_metadata, tags=url_tags
        )
        ...
        if store_metadata_separately and response_headers:
            ...
            metadata_memory_id = await memory_manager.store_memory(
                content=metadata_content, scope=scope, metadata=metadata_metadata, tags=url_tags + ["http_metadata"]
            )

            stored_memories.append({"memory_id": metadata_memory_id, "type": "http_metadata", "url": url})
...
GOOGLE_SEARCH_AND_STORE_TOOL = Tool(
    ...
    inputSchema={
        ...
        "properties": {
            ...
            "start_index": {"type": "integer", "minimum": 1, "description": "Starting index for search results"},
            "image_search": {"type": "boolean", "description": "Enable image search mode"},
            "image_size": {
                "type": "string",
            ...
            "store_summary": {"type": "boolean", "description": "Store search summary (default: true)"},
        },
        "required": ["query"],
    },
)
