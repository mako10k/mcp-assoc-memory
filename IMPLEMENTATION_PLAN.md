...
- [ ] **プロジェクト構造作成**
  ...
  - [ ] `src/mcp_assoc_memory/` - ソースディレクトリ
  - [ ] `tests/` - テストディレクトリ
  - [ ] `.gitignore` - Git除外設定
...
### 1.2 基本データモデル実装
- [ ] **`src/models/`**
  - [ ] `memory.py` - Memory, MemoryDomain, ProjectRole enums
  - [ ] `association.py` - Association model
  - [ ] `project.py` - Project, ProjectMember models
  - [ ] `schemas.py` - MCP JSON schemas
...
- [ ] **`src/handlers/tools.py`**
  ...
  - [ ] `memory_manage` ツール実装
    - [ ] stats, export, import, change_domain, batch_delete, cleanup
  - [ ] `search` ツール実装
...
- [ ] **`src/transport/`**
  ...
  - [ ] `http_handler.py` - HTTP API (FastAPI)
  - [ ] `sse_handler.py` - WebSocket/SSE通信
