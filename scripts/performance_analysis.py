        ...
        with self.measure_time("store_5_memories"):
            for i, content in enumerate(test_memories):
                memory = await memory_manager.store_memory(
                    content=content, scope=f"test/performance/{i}", tags=["performance", "test"], metadata={"index": i}
                )
                stored_memories.append(memory)

        # Measure search performance
        search_queries = ["programming language", "web framework", "vector database", "machine learning", "REST API"]
